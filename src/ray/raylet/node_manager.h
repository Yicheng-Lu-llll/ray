...
class NodeManager : public rpc::NodeManagerServiceHandler,
 ...
 private:
  ...
  /// Post one scheduling rescan for the resource-view changes consumed so far.
  /// A whole batch of syncer messages is applied inside one event-loop turn, so
  /// coalescing here means a burst of thousands of messages costs one rescan.
  ...
  /// Whether a resource-view rescan is already posted and not yet run.
  bool resource_view_rescan_posted_ = false;

  /// Whether a consumed message changed a node's capacity (totals or labels)
  /// since the last rescan. Only then can an infeasible lease have become
  /// feasible, so only then does the posted rescan include the infeasible queue.
