...
void NodeManager::ConsumeSyncMessage(
  ...
  if (message->message_type() == syncer::MessageType::RESOURCE_VIEW) {
    ...
    // Only kick lease scheduling for messages that actually change the view:
    // a message whose relevant content matches the stored view (e.g. a change
    // that canceled itself out between two reports, or a redelivery via
    // NodeAdded) cannot unblock any queued lease.
  ...
  io_service_.post(
      [this]() {
        ...
        if (rescan_infeasible) {
          ...
          // Only availability changed, which cannot have made an infeasible
          // lease feasible, so the infeasible queue does not need a rescan.
