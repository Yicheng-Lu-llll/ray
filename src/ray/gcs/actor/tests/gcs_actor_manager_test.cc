...
TEST_F(GcsActorManagerTest, TestLazyRegistrationCreateRegistersInlineAndCreates) {
  // register_if_absent set: a CreateActor for an unknown actor
  ...
  auto actor = GetRegisteredActor(*gcs_actor_manager_, actor_id);
  ...
  ASSERT_EQ(create_reply.actor_address().worker_id(), actor->GetAddress().worker_id());
}

TEST_F(GcsActorManagerTest, TestLazyRegistrationCreateDuplicateRequest) {
  ...
  create_request.set_register_if_absent(true);
  int reply_count = 0;
  auto count_replies = [&reply_count](Status,
                                      std::function<void()>,
...
TEST_F(GcsActorManagerTest, TestLazyRegistrationCreateDoesNotResurrectDeadActor) {
...
TEST_F(GcsActorManagerTest, TestReuseActorNameInNamespace) {
  ...
  Status status =
      gcs_actor_manager_->RegisterActor(request_1.task_spec(), [](const Status &) {});
  ASSERT_TRUE(status.ok());
  ...
  status =
      gcs_actor_manager_->RegisterActor(request_2.task_spec(), [](const Status &) {});
  ASSERT_TRUE(status.ok());
  ASSERT_EQ(gcs_actor_manager_->GetActorIDByName(actor_name, ray_namespace).Binary(),
            actor_id_2.Binary());
  io_service_.run_one();
...
TEST_F(GcsActorManagerTest, TestNodeFailureDestroysAllOwnedActors) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    Status status = gcs_actor_manager_->RegisterActor(register_request.task_spec(),
                                                      [](const Status &) {});
...
TEST_F(GcsActorManagerTest, TestInlineRegisterDestroyRaceDoesNotResurrectOrDoubleReply) {
  // The exact battery-found crash chain: create(register_if_absent) with the
  // inline registration still persisting -> the handle goes out of scope and
  // the actor is destroyed -> the persistence callback takes the cancel
  // branch, which must reply the create EXACTLY once and erase the callback
  // list -> a later standalone RegisterActor for the same id (the lazy escape
  // machinery can send one) must be rejected via the dead-actor cache, never
  // re-fire the stale create callback, and never re-schedule the actor.
  auto job_id = JobID::FromInt(41);
  auto register_request = GenRegisterActorRequest(job_id,
                                                  /*max_restarts=*/0,
                                                  /*detached=*/false);
  ...
  create_request.set_register_if_absent(true);
  rpc::CreateActorReply create_reply;
  int create_replies = 0;
  gcs_actor_manager_->HandleCreateActor(
      create_request,
      &create_reply,
      [&create_replies](Status, std::function<void()>, std::function<void()>) {
        create_replies++;
      });
  // Inline registration persistence is now in flight (not drained). The
  // handle goes out of scope: destroy wins the race.
  rpc::ReportActorOutOfScopeRequest oos_request;
  oos_request.set_actor_id(actor_id.Binary());
  oos_request.set_num_restarts_due_to_lineage_reconstruction(0);
  rpc::ReportActorOutOfScopeReply oos_reply;
  auto noop_reply = [](Status, std::function<void()>, std::function<void()>) {};
  gcs_actor_manager_->HandleReportActorOutOfScope(oos_request, &oos_reply, noop_reply);
  drain_io_context();
  // The create was answered exactly once (cancelled), and nothing scheduled.
  ASSERT_EQ(create_replies, 1);
  ASSERT_TRUE(mock_actor_scheduler_->actors.empty());
  ASSERT_EQ(GetRegisteredActor(*gcs_actor_manager_, actor_id), nullptr);

  // A late standalone registration for the same id must be rejected and must
  // not re-fire the (already consumed) create callback.
  rpc::RegisterActorReply rr_reply;
  int register_replies = 0;
  gcs_actor_manager_->HandleRegisterActor(
      register_request,
      &rr_reply,
      [&register_replies](Status, std::function<void()>, std::function<void()>) {
        register_replies++;
      });
  drain_io_context();
  ASSERT_EQ(register_replies, 1);
  ASSERT_NE(rr_reply.status().code(), static_cast<int>(StatusCode::OK));
  ASSERT_EQ(GetRegisteredActor(*gcs_actor_manager_, actor_id), nullptr);
  ASSERT_TRUE(mock_actor_scheduler_->actors.empty());
  ASSERT_EQ(create_replies, 1);
}
