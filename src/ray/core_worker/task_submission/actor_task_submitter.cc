...
void ActorTaskSubmitter::SubmitActorCreationTask(TaskSpecification task_spec) {
  ...
  if (::RayConfig::instance().actor_ref_deleted_push_enabled() &&
    ...
    // This is called after the actor registration reply is received (for both
    // the sync and async registration paths), so the ref-deleted report can
    // never reach the GCS before the registration does. Unlike the out-of-scope
    // callback (installed once per actor queue), this has no install-once guard:
    // a resubmission of the creation task would install a second callback and
    // produce a duplicate report, which the GCS handler treats as a no-op.
